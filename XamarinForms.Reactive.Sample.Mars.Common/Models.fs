...
type RoverSolPhotoSet =
    {
        ...
        mutable VisibleCameras: string[]
        mutable Command: ICommand
...
type PhotoSet =
    {
        mutable Camera: string
        mutable Sol: int
        [<JsonProperty("photos")>] mutable Photos: Photo[]
    }

module RoverCameras =
    let private fhaz = { Name = "FHAZ"; FullName = "Front Hazard Avoidance" }
    let private rhaz = { Name = "RHAZ"; FullName = "Rear Hazard Avoidance" }
    let private mast = { Name = "MAST"; FullName = "Mast Camera" }
    let private chemCam = { Name = "CHEMCAM"; FullName = "Chemistry/Camera Complex" }
    let private entry = { Name = "ENTRY"; FullName = "Entry" }
    ...
    let private minites = { Name = "MINITES"; FullName = "Mini Thermal Emission Spectrometer" }
    let all = dict [(fhaz.Name, fhaz); (rhaz.Name, rhaz); (mast.Name, mast); (chemCam.Name, chemCam); (entry.Name, entry); (mahli.Name, mahli); (mardi.Name, mardi); (navCam.Name, navCam); (panCam.Name, panCam); (minites.Name, minites)]
